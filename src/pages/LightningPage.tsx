        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12 sm:mb-16">
                {FEATURES.map((f) => (
                    <div
                        key={f.title}
                        className="bg-white rounded-2xl p-6 sm:p-7 flex flex-col gap-3 border border-black/5"
                    >
                        ...
                        <p className="text-black/70 text-sm sm:text-base leading-relaxed">{f.body}</p>
                    </div>
                ))}
            </div>

            {/* Sample invoice */}
            <div className="bg-black rounded-2xl p-6 sm:p-8 md:p-10 mb-12 sm:mb-16">
                <p className="text-[#F7931A] text-xs sm:text-sm uppercase tracking-wider mb-3">
                    Example BOLT11 invoice
                </p>
                <code
                    className="block text-[#F5F5F5] text-sm sm:text-base md:text-lg break-all"
                    style={{ fontFamily: '"Courier New", monospace' }}
                >
                    {SAMPLE_INVOICE}
                </code>
                <p className="text-[#F5F5F5]/60 text-xs sm:text-sm mt-4 max-w-2xl">
            ...
            <div className="mb-12 sm:mb-16">
                ...
                <p className="text-[#F5F5F5]/80 text-sm sm:text-base max-w-2xl mb-6 sm:mb-8">
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3">
                    {WALLETS.map((w) => (
                        <span
                            key={w}
                            className="px-4 sm:px-5 py-2 rounded-full bg-white text-black text-xs sm:text-sm font-medium border border-black/10"
                        >
                            {w}
                        </span>
                    ))}
